...
object exercises extends App {
  ...
  //
  def crawlIOWillCycle[E: Monoid, A: Monoid](
                                     seeds     : Set[URL],
                                     router    : URL => Set[URL],
                                     processor : (URL, String) => IO[E, A]): IO[Nothing, Crawl[E, A]] =
    IO.traverse(seeds)(url =>
      getURL(url).redeem(
        // We don't have an Exception => E so we have to throw away the error.
        _ => IO.now(mzero[Crawl[E, A]]),
        html =>
          for {
            crawl <- processor(url, html)
              .redeemPure(
                // mzero is the Monoid#empty value
                Crawl(_, mzero[A]),
                Crawl(mzero[E], _)
              )
            childUrls = extractURLs(url, html).toSet.flatMap(router)
            childCrawl <- crawlIO(childUrls, router, processor)
          } yield crawl |+| childCrawl
      )
    ).map(_.foldMap()) // #foldMap can combine the Crawls because it's a Monoid

  def crawlIONotTailRec[E: Monoid, A: Monoid](
    seeds     : Set[URL],
    router    : URL => Set[URL],
    processor : (URL, String) => IO[E, A]): IO[Nothing, Crawl[E, A]] = {

    //def getUrlPair(url: URL): IO[Nothing, Either[Exception, (URL, String)]] =
    //  getURL(url).map(url -> _)

    def recurse(seeds: Set[URL], crawl: Crawl[E, A], visited: Set[URL]): IO[Nothing, Crawl[E, A]] =
      IO.traverse(seeds)(url =>
        getURL(url).redeem(
          // We don't have an Exception => E so we have to throw away the error.
          _ => IO.now(mzero[Crawl[E, A]]),
          html =>
            for {
              ...
              childUrls = extractURLs(url, html).toSet.flatMap(router)
              childCrawl <- recurse(childUrls -- visited, crawl, visited ++ childUrls)
            } yield crawl |+| childCrawl
        )
      ).map(_.foldMap()) // #foldMap can combine the Crawls because it's a Monoid

    recurse(seeds, mzero[Crawl[E, A]], Set.empty)
  }
  ...
  def crawlIO[E: Monoid, A: Monoid](
    ...
    // Almost tail recursive. Need to replace traverse with something else so we don't need to call #void
  ...
  def crawlIOPar[E: Monoid, A: Monoid](
    ...
    def loop(seeds: Set[URL], ref: Ref[(Crawl[E, A], Set[URL])]): IO[Nothing, Unit] =
    // The change from above is #parTraverse instead of #traverse. The hard part was using Ref, which the single threaded version doesn't need.
  ...
  def myGame[F[_]: Console: Random: Monad]: F[Unit] =
    for {
      name <- getName[F]
      word <- chooseWord[F]
      state = State(name, Set.empty, word)
      _ <- gameLoop[F](state)
    } yield ()
  ...
  def gameLoop[F[_]: Console: Monad](state: State): F[State] =
    for {
      ...
      nextState = state.copy(guesses = state.guesses + guess)

      _ <- if (nextState.playerLost)
             printLine[F](s"Sorry, ${state.name}, you lost!") *> nextState.point[F]
           else if(nextState.playerWon)
             printLine[F](s"Congratulations, ${state.name}, you won!! You solved ${state.word}") *> nextState.point[F]
           else
             printLine[F](
               if (state.guesses.contains(guess)) "Stop being silly!"
               else if (state.word.contains(guess)) "Good job, keep going!"
               else "No luck, but you still have a chance!"
             ) *> gameLoop[F](nextState)
    } yield state
  ...
  def getChoice[F[_]: Console: Monad]: F[Char] =
    for {
      ...
      letter <- line.trim.toLowerCase.toList match {
        case letter :: Nil if letter isLetter =>
          Monad[F].point(letter)
        case _ =>
  ...
  def getName[F[_]: Console: Apply]: F[String] =
    printLine[F]("Enter your name:") *> readLine[F]

  def chooseWord[F[_]: Random: Functor]: F[String] =
    Random[F].nextInt(Dictionary.length).map(idx =>
      Dictionary.lift(idx).getOrElse("buggy")
    )
  ...
  def createTestInstance[E](ref: Ref[TestData]): GameEffects[TestIO[E, ?]] =
    ...
    new Console[TestIO[E, ?]] with Random[TestIO[E, ?]] with Monad[TestIO[E, ?]] {
      ...
      def printLine(line: String): TestIO[E,Unit] =
        TestIO(
          ref.update(
            d => d.copy(output = line:: d.output)
          ).void
        )
      def readLine: TestIO[E, String] =
        TestIO(
          ref.modify(d => (d.input.head, d.copy(input = d.input.drop(1))))
        )
      def nextInt(max: Int): TestIO[E,Int] =
        TestIO(
          ref.modify(d => (d.random.head, d.copy(random = d.random.drop(1))))
        )
    }
  ...
  def testGame(data: TestData): IO[Nothing, TestData] =
    for {
      ref   <- Ref(data)
      _     <- ({
                  implicit val F: GameEffects[TestIO[Nothing, ?]] = createTestInstance[Nothing](ref)
